...
import {
  ...
  RotateCcw, ShieldAlert, Lock, Camera, Trash2, Plus
} from 'lucide-react';
...
const Editor = ({ currentProject, updateField, setCurrentDeck, currentDeck, setView, saveProject }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (files.length + currentFotos.length > 4) {
      alert("Limite máximo de 4 fotos.");
      return;
    }
      ...
      case 0:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div className="col-span-1 md:col-span-2">
              <RenderInput
                label="Número do Processo SEI" field="numeroProcessoSEI" placeholder="000.0000.000000/0000-00" required
                value={currentProject.numeroProcessoSEI} onChange={(val) => updateField('numeroProcessoSEI', val)} onFocus={() => handleFieldFocus('numeroProcessoSEI')} onBlur={handleFieldBlur}
                disabled={isReadOnly}
              />
            </div>
            ...
            <div className="col-span-1 md:col-span-2">
              <RenderSelect
                label="Tipo de Instrumento" field="tipoInstrumento" options={INSTRUMENTOS_OPTIONS} required
                value={currentProject.tipoInstrumento} onChange={(val) => updateField('tipoInstrumento', val)} onFocus={() => handleFieldFocus('tipoInstrumento')} onBlur={handleFieldBlur}
                disabled={isReadOnly}
              />
            </div>

            {currentProject.tipoInstrumento === 'Outros' && (
              <div className="col-span-1 md:col-span-2 animate-in fade-in slide-in-from-top-2">
                <RenderInput
                  label="Especifique o Instrumento" field="instrumentoOutros" placeholder="Digite o tipo de instrumento" required
                  value={currentProject.instrumentoOutros} onChange={(val) => updateField('instrumentoOutros', val)} onFocus={() => handleFieldFocus('instrumentoOutros')} onBlur={handleFieldBlur}
                  disabled={isReadOnly}
                />
              </div>
            )}
            ...
            <div className="col-span-1">
              <RenderInput
                label="Número do Instrumento" field="numeroInstrumento" placeholder="Nº 000/202X" required
                value={currentProject.numeroInstrumento} onChange={(val) => updateField('numeroInstrumento', val)} onFocus={() => handleFieldFocus('numeroInstrumento')} onBlur={handleFieldBlur}
                disabled={isReadOnly}
              />
            </div>
            ...
            <div className="col-span-1">
              <RenderInput
                label="Vigência" field="vigenciaInstrumento" placeholder="DD/MM/AAAA a DD/MM/AAAA" required
                value={currentProject.vigenciaInstrumento} onChange={(val) => updateField('vigenciaInstrumento', val)} onFocus={() => handleFieldFocus('vigenciaInstrumento')} onBlur={handleFieldBlur}
                disabled={isReadOnly}
              />
            </div>
            <div className="col-span-1 md:col-span-2 py-4">
              <div className="checkbox-wrapper">
                <input
                  ...
                  className="check"
                  id="instrumentoAtivo"
                  checked={currentProject.instrumentoAtivo || false}
                  onChange={(e) => updateField('instrumentoAtivo', e.target.checked)}
                  disabled={isReadOnly}
                />
                <label htmlFor="instrumentoAtivo" className="label">
                  ...
                  <span className="text-xs font-black text-slate-700 uppercase">Instrumento Ativo / Vigente</span>
                </label>
              </div>
            </div>
            <div className="col-span-1 md:col-span-2">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <RenderInput label="Nmr. Beneficiários" field="nmrBeneficiarios" type="number" required value={currentProject.nmrBeneficiarios} onChange={(val) => updateField('nmrBeneficiarios', val)} onFocus={() => handleFieldFocus('nmrBeneficiarios')} onBlur={handleFieldBlur} disabled={isReadOnly} />
            </div>
            ...
            <RenderInput label="Local de Execução (Detalhes)" field="localExecucao" required value={currentProject.localExecucao} onChange={(val) => updateField('localExecucao', val)} onFocus={() => handleFieldFocus('localExecucao')} onBlur={handleFieldBlur} disabled={isReadOnly} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <RenderSelect label="Território" field="territorio" options={TERRITORIOS_BAHIA} required value={currentProject.territorio} onChange={(val) => updateField('territorio', val)} onFocus={() => handleFieldFocus('territorio')} onBlur={handleFieldBlur} disabled={isReadOnly} />
              <RenderSelect label="Município" field="municipio" options={MUNICIPIOS_BAHIA} required value={currentProject.municipio} onChange={(val) => updateField('municipio', val)} onFocus={() => handleFieldFocus('municipio')} onBlur={handleFieldBlur} disabled={isReadOnly} />
            </div>
          </div>
        );
      case 3:
        return (
          <div className="space-y-6">
            ...
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-6">
              ...
              <RenderSelect label="Fonte de Financiamento" field="fonteFinanciamento" options={FONTES_FINANCIAMENTO} value={currentProject.fonteFinanciamento} onChange={(val) => updateField('fonteFinanciamento', val)} onFocus={() => handleFieldFocus('fonteFinanciamento')} onBlur={handleFieldBlur} disabled={isReadOnly} />

              <RenderInput label="Descrição dos Recursos Financeiros" field="recursosFinanceiros" height="h-24" value={currentProject.recursosFinanceiros} onChange={(val) => updateField('recursosFinanceiros', val)} onFocus={() => handleFieldFocus('recursosFinanceiros')} onBlur={handleFieldBlur} disabled={isReadOnly} />
            </div>
          </div>
        );
      case 4:
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Camera size={14} /> Registros Fotográficos (Máx. 4)
                  </h3>
                  <span className="text-[10px] font-bold text-slate-400 bg-slate-200 px-2 py-1 rounded-md">{currentProject.fotos ? currentProject.fotos.length : 0}/4</span>
                </div>

                {!isReadOnly && (currentProject.fotos?.length || 0) < 4 && (
                  <div className="mb-6">
    ...
    <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-4 lg:gap-8 relative pb-20 lg:pb-0">
      ...
      >
        ...
        >
          ...
          <svg onClick={() => setActiveHelp(null)} className="w-[18px] h-[18px] text-slate-400 cursor-pointer hover:text-slate-600 transition-colors shrink-0" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="mt-12 pt-6 border-t border-slate-100">
            <div className="flex flex-col md:flex-row justify-between gap-4">
              ...
              <div className="flex gap-2 order-1 md:order-2">
                ...
                {/* CORREÇÃO AQUI: currentDeck < 5 (antes estava < 6) */}
                {currentDeck < 5 ? (
